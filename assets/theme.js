    ...
    Updater: (function () {
      class Updater {
        ...
        update(targetElement) {
          ...
          const overflowX = this.el.scrollWidth - this.el.offsetWidth;
          const overflowY = this.el.scrollHeight - this.el.offsetHeight;
          const maxScrollX = overflowX > 2 ? overflowX : 0;
          const maxScrollY = overflowY > 2 ? overflowY : 0;
          const sx = maxScrollX > 0 ? Math.abs(scrollLeft) / maxScrollX : 0;
          const sy = maxScrollY > 0 ? scrollTop / maxScrollY : 0;
          const sw = maxScrollX > 0 ? this.el.offsetWidth / this.el.scrollWidth : 1;
          const sh = maxScrollY > 0 ? this.el.offsetHeight / this.el.scrollHeight : 1;
          const tx = sx * (1 - sw);
          const ty = sy * (1 - sh);
          let cssText = `--t: ${Math.floor(scrollTop) > 0 ? 1 : 0}; --b: ${Math.floor(scrollBottom) > 0 ? 1 : 0}; --l: ${Math.floor(scrollLeft) > 0 ? 1 : 0}; --r: ${Math.floor(scrollRight) > 0 ? 1 : 0}; --sx: ${sx}; --sy: ${sy}; --sw: ${sw}; --sh: ${sh}; --tx: ${tx}; --ty: ${ty};`;
...
class SecondaryMedia extends HTMLElement {
  ...
  init() {
    ...
    this.mediaItems = this.querySelectorAll('.media');
    this.mediaCount = this.mediaItems.length;

    if (this.mediaCount > 1) {
      this.hoverIndex = 1;
      this._isHoverActive = false;
      this.mediaItems[0].classList.add('is-active');
      ...
      this._scrollHandler = () => {
        ...
        this._scrollRaf = requestAnimationFrame(() => {
          ...
          if (hovered && !this._isHoverActive) {
            this.onEnterHandler();
          } else if (!hovered && this._isHoverActive) {
            this.onLeaveHandler();
          }
        });
      };
  ...
  onEnterHandler() {
    ...
    this.mediaItems[0].classList.remove('is-active');
    this.mediaItems[this.hoverIndex].classList.add('is-active');
  }

  onLeaveHandler() {
    this._isHoverActive = false;
    this.mediaItems[this.hoverIndex].classList.remove('is-active');
    this.mediaItems[0].classList.add('is-active');
  }
}
